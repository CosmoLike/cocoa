#!/bin/bash
# ------------------------------------------------------------------------------
# setup_cosmolike.sh: download the cosmolike core libraries (cfastpt, cfftlog,
# ...).
#
...
#
# Skipped (return 99) when IGNORE_COSMOLIKE_CODE is set. Reruns are safe:
# existing results are kept unless OVERWRITE_EXISTING_COSMOLIKE_CODE is set.
...
if [ ! -d "${PACKDIR:?}" ]; then
  ...
  if [ -n "${COSMOLIKE_GIT_COMMIT:-}" ]; then
    ...
    "${GIT:?}" checkout "tags/${COSMOLIKE_GIT_TAG:?}" -b "${COSMOLIKE_GIT_TAG:?}TMP" \
      >>${OUT1:?} 2>>${OUT2:?} || { error "${EC16:?}"; return 1; }
  fi
