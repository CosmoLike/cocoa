#!/bin/bash
# ------------------------------------------------------------------------------
# unxv_cosmopower.sh: unpack CosmoPower (neural-network emulators for CMB and
# matter power spectra) into external_modules/data.
#
...
#
# Skipped (return 99) when IGNORE_COSMOPOWER_DATA is set. Reruns are safe:
# existing results are kept unless OVERWRITE_EXISTING_COSMOPOWER_DATA is set.
...
# ------------------------------------------------------------------------------
if [ -n "${IGNORE_COSMOPOWER_DATA:-}" ]; then
  return 99
fi
...
# ---------------------------------------------------------------------------
ptop "SETUP/UNXV EMULATOR CMB COSMOPOWER DATA" || { unset_all; return 1; }
...
pbottom "SETUP/UNXV EMULATOR CMB COSMOPOWER DATA" || { unset_all; return 1; }
