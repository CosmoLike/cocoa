#!/bin/bash
# ------------------------------------------------------------------------------
# compile_fbre.sh: build and install the FLAMINGO baryonic response emulator
# (fbre).
#
...
#
# Skipped (return 99) when IGNORE_FBRE_CODE is set. On failure error() names
...
ptop "COMPILING BARYONIC FBRE FEEDBACK MODELS" || { unset_all; return 1; }

# ----------------------------------------------------------------------------
# cleaning any previous compilation
# ----------------------------------------------------------------------------
rm -rf "${PACKDIR:?}/build/"
rm -rf "${PLIB:?}"/FlamingoBaryonResponseEmulator/
rm -rf "${PLIB:?}"/FlamingoBaryonResponseEmulator-*
rm -rf "${PLIB:?}"/flamingobaryonresponseemulator-*
# ----------------------------------------------------------------------------
...
pbottom "COMPILING BARYONIC FBRE FEEDBACK MODELS" || { unset_all; return 1; }
