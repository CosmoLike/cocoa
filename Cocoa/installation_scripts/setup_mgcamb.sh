#!/bin/bash
# ------------------------------------------------------------------------------
# setup_mgcamb.sh: download MGCAMB (modified-gravity CAMB).
#
...
# happens here.
#
# Skipped (return 99) when IGNORE_MGCAMB_CODE is set. Reruns are safe:
# existing results are kept unless OVERWRITE_EXISTING_MGCAMB_CODE is set. On
...
if [ ! -d "${PACKDIR:?}" ]; then
  ...
  cdfolder "${ECODEF:?}/${TFOLDER:?}" || return 1;

  if [[ -n "${MGCAMB_GIT_COMMIT:-}" ||
        -n "${MGCAMB_GIT_BRANCH:-}" ||
        -n "${MGCAMB_GIT_TAG:-}" ]]; then
  ...
  if [ -n "${MGCAMB_GIT_COMMIT:-}" ]; then
    ...
    "${GIT:?}" checkout "tags/${MGCAMB_GIT_TAG:?}" -b "${MGCAMB_GIT_TAG:?}TMP" \
      >>${OUT1:?} 2>>${OUT2:?} || { error "${EC16:?}"; return 1; }
  fi
