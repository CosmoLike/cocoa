...
if [ -z "${IGNORE_CMAKE_INSTALLATION}" ]; then
    ...
    git clone https://github.com/Kitware/CMake.git cmake-3.26.4 > ${OUTPUT_SIL_1} 2> ${OUTPUT_SIL_2}
    if [ $? -ne 0 ];then
    ...
    git checkout v3.26.4 > ${OUTPUT_SIL_1} 2> ${OUTPUT_SIL_2}
    if [ $? -ne 0 ];then
...
if [ -z "${IGNORE_OPENBLAS_INSTALLATION}" ]; then
    ...
    git clone https://github.com/OpenMathLib/OpenBLAS.git OpenBLAS-0.3.23 > ${OUTPUT_SIL_1} 2> ${OUTPUT_SIL_2}
    if [ $? -ne 0 ];then
    ...
    git checkout v0.3.23 > ${OUTPUT_SIL_1} 2> ${OUTPUT_SIL_2}
    if [ $? -ne 0 ];then
...
if [ -z "${IGNORE_FORTRAN_LAPACK_INSTALLATION}" ]; then
    ...
    git clone https://github.com/Reference-LAPACK/lapack.git lapack > ${OUTPUT_SIL_1} 2> ${OUTPUT_SIL_2}
    if [ $? -ne 0 ];then
    ...
    git checkout lapack-3.11.0 > ${OUTPUT_SIL_1} 2> ${OUTPUT_SIL_2}
    if [ $? -ne 0 ];then
...
if [ -z "${IGNORE_CPP_SPDLOG_INSTALLATION}" ]; then
    ...
    git clone https://github.com/gabime/spdlog.git spdlog > ${OUTPUT_SIL_1} 2> ${OUTPUT_SIL_2}
    if [ $? -ne 0 ];then
    ...
    git checkout v1.13.0 > ${OUTPUT_SIL_1} 2> ${OUTPUT_SIL_2}
    if [ $? -ne 0 ];then
...
if [ -z "${IGNORE_CPP_CARMA_INSTALLATION}" ]; then
    ...
    git clone https://github.com/RUrlus/carma.git carma_tmp > ${OUTPUT_SIL_1} 2> ${OUTPUT_SIL_2}
    if [ $? -ne 0 ];then
    ...
    git checkout v0.7.0 > ${OUTPUT_SIL_1} 2> ${OUTPUT_SIL_2}
    if [ $? -ne 0 ];then
