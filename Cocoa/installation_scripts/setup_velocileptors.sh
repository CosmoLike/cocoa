#!/bin/bash
# ------------------------------------------------------------------------------
# setup_velocileptors.sh: download velocileptors (Lagrangian perturbation
# theory for redshift-space spectra).
#
...
#
# Skipped (return 99) when IGNORE_VELOCILEPTORS_CODE is set. Reruns are safe:
# existing results are kept unless OVERWRITE_EXISTING_VELOCILEPTORS_CODE is
...
if [ ! -d "${PACKDIR:?}" ]; then
  ...
  if [ -n "${VELOCILEPTORS_GIT_COMMIT:-}" ]; then
    ...
    "${GIT:?}" checkout "tags/${VELOCILEPTORS_GIT_TAG:?}" -b "${VELOCILEPTORS_GIT_TAG:?}TMP" \
      >>${OUT1:?} 2>>${OUT2:?} || { error "${EC16:?}"; return 1; }
  fi
