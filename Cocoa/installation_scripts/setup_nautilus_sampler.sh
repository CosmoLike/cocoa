#!/bin/bash
# ------------------------------------------------------------------------------
# setup_nautilus_sampler.sh: download the Nautilus nested sampler.
#
...
#
# Skipped (return 99) when IGNORE_NAUTILUS_SAMPLER_CODE is set. Reruns are
# safe: existing results are kept unless OVERWRITE_EXISTING_NAUTILUS_CODE is
...
if [ ! -d "${PACKDIR:?}" ]; then
  ...
  if [ -n "${NAUTILUS_GIT_COMMIT:-}" ]; then
    ...
    "${GIT:?}" checkout "tags/${NAUTILUS_GIT_TAG:?}" -b "${NAUTILUS_GIT_TAG:?}TMP" \
      >>${OUT1:?} 2>>${OUT2:?} || { error "${EC16:?}"; return 1; }
  fi
