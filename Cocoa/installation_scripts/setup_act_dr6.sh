#!/bin/bash
# ------------------------------------------------------------------------------
# setup_act_dr6.sh: download the ACT DR6 CMB lensing likelihood code.
#
...
# happens here.
#
# Skipped (return 99) when IGNORE_ACTDR6_CODE is set. Reruns are safe:
# existing results are kept unless OVERWRITE_EXISTING_ACTDR6_CMB_CODE is set.
...
if [[ ! -d "${PACKDIR:?}" ]]; then
  ...
  if [[ -n "${ACTDR6_GIT_COMMIT:-}" ||
        -n "${ACTDR6_GIT_BRANCH:-}" ||
        -n "${ACTDR6_GIT_TAG:-}" ]]; then
