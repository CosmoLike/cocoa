#!/bin/bash
# ------------------------------------------------------------------------------
# compile_pyfastpt.sh: build and install FAST-PT (perturbation-theory
# integrals in python).
#
...
#
# Skipped (return 99) when IGNORE_FASTPT_CODE is set. On failure error() names
...
# ----------------------------------------------------------------------------
rm -rf "${PACKDIR:?}/build/"
rm -rf "${PACKDIR:?}/fast_pt.egg-info/"
# ---------------------------------------------------------------------------  
