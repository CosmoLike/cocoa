#!/bin/bash
# ------------------------------------------------------------------------------
# setup_ee2.sh: download EuclidEmulator2 (a nonlinear matter-power boost
# emulator).
#
...
# happens here.
#
# Skipped (return 99) when IGNORE_EUCLID_EMULATOR_V2_CODE is set. Reruns are
# safe: existing results are kept unless OVERWRITE_EXISTING_EE2_CODE is set.
...
# ------------------------------------------------------------------------------
if [ -n "${IGNORE_EUCLID_EMULATOR_V2_CODE:-}" ]; then
  return 99
fi
...
if [[ ! -d "${PACKDIR:?}" ]]; then
  ...
  if [ -n "${EE2_GIT_COMMIT:-}" ]; then
    ...
    "${GIT:?}" checkout "tags/${EE2_GIT_TAG:?}" -b "${EE2_GIT_TAG:?}TMP" \
      >>${OUT1:?} 2>>${OUT2:?} || { error "${EC16:?}"; return 1; }
  fi
