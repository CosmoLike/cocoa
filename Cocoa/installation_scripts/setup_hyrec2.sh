#!/bin/bash
# ------------------------------------------------------------------------------
# setup_hyrec2.sh: download HYREC-2 (a fast recombination code).
#
...
# happens here.
#
# Skipped (return 99) when IGNORE_HYREC_CODE is set. Reruns are safe: existing
# results are kept unless OVERWRITE_EXISTING_HYREC_CODE is set. On failure
...
if [ ! -d "${PACKDIR:?}" ]; then
  ...
  if [ -n "${HYREC_GIT_COMMIT:-}" ]; then
    ...
    "${GIT:?}" checkout "tags/${HYREC_GIT_TAG:?}" -b "${HYREC_GIT_TAG:?}TMP" \
      >>${OUT1:?} 2>>${OUT2:?} || { error "${EC16:?}"; return 1; }
  fi
