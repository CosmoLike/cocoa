#!/bin/bash
# ------------------------------------------------------------------------------
# setup_getdist.sh: download GetDist (MCMC sample analysis and plotting).
#
...
#
# Skipped (return 99) when IGNORE_GETDIST_CODE is set. Reruns are safe:
# existing results are kept unless OVERWRITE_EXISTING_GETDIST_CODE is set. On
...
if [ ! -d "${PACKDIR:?}" ]; then
  ...
  if [[ -n "${GETDIST_GIT_COMMIT:-}" ||
        -n "${GETDIST_GIT_BRANCH:-}" ||
        -n "${GETDIST_GIT_TAG:-}" ]]; then
  ...
  if [ -n "${GETDIST_GIT_COMMIT:-}" ]; then
    "${GIT:?}" checkout "${GETDIST_GIT_COMMIT:?}" \
      >>${OUT1:?} 2>>${OUT2:?} || { error "${EC16:?}"; return 1; }
  elif [ -n "${GETDIST_GIT_BRANCH:-}" ]; then
    "${GIT:?}" checkout -b "${GETDIST_GIT_BRANCH:?}" "origin/${GETDIST_GIT_BRANCH:?}" \
      >>${OUT1:?} 2>>${OUT2:?} || { error "${EC16:?}"; return 1; }
  elif [ -n "${GETDIST_GIT_TAG:-}" ]; then
    "${GIT:?}" checkout "tags/${GETDIST_GIT_TAG:?}" -b "${GETDIST_GIT_TAG:?}TMP" \
      >>${OUT1:?} 2>>${OUT2:?} || { error "${EC16:?}"; return 1; }
  fi
