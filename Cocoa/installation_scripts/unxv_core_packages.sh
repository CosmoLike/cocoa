#!/bin/bash
# ------------------------------------------------------------------------------
# unxv_core_packages.sh: unpack the compiled core libraries Cocoa ships (fftw,
# cfitsio, gsl, spdlog, armadillo, boost, carma, ...) into
...
#
# Skipped (return 99) when IGNORE_CORE_INSTALLATION is set. Reruns are safe:
# existing results are kept unless OVERWRITE_EXISTING_CORE_PACKAGES is set. On
...
# ----------------------------------------------------------------------------
# ----------------------------------------------------------------------------
if [[ -n "${SKIP_DECOMM_CORE_PACKAGES:-}" ]]; then
  return 99
fi
