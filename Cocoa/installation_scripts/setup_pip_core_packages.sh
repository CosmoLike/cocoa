#!/bin/bash
# ------------------------------------------------------------------------------
# setup_pip_core_packages.sh: download the pinned python packages Cocoa
# installs into .local.
#
...
# happens here.
#
# Skipped (return 99) when IGNORE_PIP_CORE_INSTALLATION is set. Reruns are
# safe: existing results are kept unless OVERWRITE_EXISTING_PIP_PACKAGES is
...
# ------------------------------------------------------------------------------
if [ -n "${IGNORE_PIP_CORE_INSTALLATION:-}" ]; then
  return 99
fi
