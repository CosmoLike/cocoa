...
void J_abl_ar(double *x, double *fx, long N, int *alpha, int *beta, int *ell, int *isP13type, int Nterms, fastpt_config *config, double **Fy) {
	...
	for(i_term=0; i_term<Nterms; i_term++) {
		...
		if(alpha != beta){
			...
			fftconvolve_optimize(out_pad1[i_term], out_pad2[i_term], N+1, pads_convolve, a[i_term], b[i_term], a1[i_term], b1[i_term], c[i_term], pa[i_term], pb[i_term], pc[i_term]);
		}else{
			fftconvolve_optimize(out_pad1[i_term], out_pad1[i_term], N+1, pads_convolve, a[i_term], b[i_term], a1[i_term], b1[i_term], c[i_term], pa[i_term], pb[i_term], pc[i_term]);
		}
		...
		for(long i=0; i<N_original; i++){
			Fy[i_term][i] = out_ifft[i_term][2*(i+N_pad+N_extrap_low)] * sign_ell / (M_PI*M_PI) * pow(2., 2.+2*config->nu+alpha[i_term]+beta[i_term]) * pow(x[i],-p-2.);
		}
