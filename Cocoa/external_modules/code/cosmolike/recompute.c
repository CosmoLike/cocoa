...
int recompute_zphot_magnification(nuisancepara N) {
  ...
  int i, res = 0;
  for (i = 0; i < tomo.magnification_Nbin; i++) {
    if (N.sigma_zphot_magnification[i] !=
...
int recompute_galaxies(galpara G, int i) {
  ...
  if (G.hod[i][0] > 10 && G.hod[i][0] < 16) {
    for (j = 0; j < 6; j++) {
      if (G.hod[i][j] != gbias.hod[i][j]) {
        return 1;
      }
    }
  }
  if (G.b[i] != gbias.b[i] || G.b2[i] != gbias.b2[i] ||
      G.bs2[i] != gbias.bs2[i] || G.cg[i] != gbias.cg[i]) {
    return 1;
  }
  return 0;
}
...
int recompute_ggl(cosmopara C, galpara G, nuisancepara N, int i) {
  if (recompute_cosmo3D(C) || recompute_zphot_clustering(N) ||
      recompute_zphot_shear(N) || recompute_galaxies(G, i) || recompute_IA(N)) {
    return 1;
  } else {
    return 0;
  }
