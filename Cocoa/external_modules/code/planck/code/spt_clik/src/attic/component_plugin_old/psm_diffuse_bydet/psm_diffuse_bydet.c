  ...
  for (ell=egl->lmin;ell<=egl->lmax;ell++) {
    ...
    for (m1=0;m1<ndet;m1++) {
      for (m2=m1;m2<ndet;m2++) {
        ...
        Rq[IDX_R(egl,ell,m1,m2)] = psm_diffuse_bydet_norm *1e12* A[mell_in + m1*ndets_hfi + m2];
...
parametric *psm_diffuse_bydet_init(int ndet, double *detlist, int ndef, char** defkey, char **defvalue, int nvar, char **varkey, int lmin, int lmax, double* template, error **err) {
  parametric *egl;
  pfchar type;
  char *pt;
  int lmax_in=3000;
